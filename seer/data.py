...
def classify_dataset(args, trainset, testset,prop1, w1=1.0, prop2 = None, w2 = 0.0, prop3 = None, w3 = 0.0):
    ...
    total_score = [0] * len(trainset)

    print("trainset len = ", len(trainset))
    print("datapoints len = ", len(datapoints))
    print("score1 len = ", len(score1))
    print("score2 len = ", len(score2))
    print("score3 len = ", len(score3))
    for i in range(len(datapoints)):
        total_score[i] = score1[i]*w1+score2[i]*w2+score3[i]*w3 
    ...
    score2 = []
    score3 = []
    datapoints = [testset[i][0].unsqueeze(0) for i in range(len(testset))]
    for datapoint in datapoints:
        score1.append(property_scores(datapoint, prop1))
    score1 = normalize_scores(score1)
    if prop2 is not None:
